...
resource "oci_core_virtual_network" "dorinVCN" {
...
resource "oci_core_internet_gateway" "IG1" {
    ...
    vcn_id = "${oci_core_virtual_network.dorinVCN.id}"
    defined_tags = {"BUCH_IaaS.PersonalLearning"="Dorin"}
}
...
resource "oci_core_subnet" "WBA" {
  ...
  dhcp_options_id = "${oci_core_virtual_network.dorinVCN.default_dhcp_options_id}"
  defined_tags = {"BUCH_IaaS.PersonalLearning"="Dorin"}
}
...
resource "oci_core_subnet" "WBA2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}" 
  cidr_block = "${var.WBA2_subnet_cidr}"
  display_name = "WBA2"
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${oci_core_virtual_network.dorinVCN.id}"
  route_table_id = "${oci_core_route_table.RouteFordorinVCN.id}"
...
resource "oci_core_subnet" "WBB2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[1],"name")}" 
  cidr_block = "${var.WBB2_subnet_cidr}"
  display_name = "WBB2"
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${oci_core_virtual_network.dorinVCN.id}"
  route_table_id = "${oci_core_route_table.RouteFordorinVCN.id}"
...
#define Load Balancer subnets. Select one of three availability domains by using [0, 1, 2] in the "availabity_domain" line
resource "oci_core_subnet" "LBA2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}" 
  cidr_block = "${var.LBA2_subnet_cidr}"
  display_name = "LBA2"
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${oci_core_virtual_network.dorinVCN.id}"
  route_table_id = "${oci_core_route_table.RouteFordorinVCN.id}"
...
resource "oci_core_subnet" "LBB2" {
  availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[1],"name")}" 
  cidr_block = "${var.LBB2_subnet_cidr}"
  display_name = "LBB2"
  compartment_id = "${var.compartment_ocid}"
  vcn_id = "${oci_core_virtual_network.dorinVCN.id}"
  route_table_id = "${oci_core_route_table.RouteFordorinVCN.id}"
